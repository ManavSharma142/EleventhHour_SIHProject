...
export default function Nutrition() {
  ...
  const getflexcoins = async (username) => {
    ...
    console.log(`Fetching flexcoins for ${username}`);
    return;
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#0A0F1C] via-[#0D1421] to-[#111827] text-white flex flex-col lg:flex-row relative overflow-hidden">
      ...
      ">
        {/* Logo with animation - Hidden on mobile */}
        ...
        {/* Enhanced Navigation - Adapts to row for mobile */}
        <nav className="flex-1 lg:space-y-3 flex flex-row lg:flex-col justify-around items-center w-full">
          {[
            ...
            <Link
              to={page}
              key={label}
              className={`group flex lg:flex-row flex-col items-center gap-1 lg:gap-4 lg:px-5 lg:py-4 p-2 rounded-2xl cursor-pointer transition-all duration-300 relative overflow-hidden w-1/5 lg:w-full
                ${active
                  ? "bg-gradient-to-r from-blue-600/20 to-purple-600/20 lg:border border-blue-500/30 shadow-lg text-white"
                  : "text-gray-300 hover:text-white hover:bg-white/5 lg:border border-transparent hover:border-white/10"
                }`}
            >
              {active && <div className="absolute inset-0 bg-gradient-to-r from-blue-600/10 to-purple-600/10 rounded-2xl"></div>}
              <div className={`relative p-2 rounded-xl ${active ? `bg-gradient-to-r ${color}` : 'bg-gray-700/50 group-hover:bg-gray-600/50'} transition-all duration-300`}>
                <Icon className="w-5 h-5 relative z-10" />
              </div>
              <span className="text-xs lg:text-base font-semibold relative z-10">{label}</span>
              {active && <div className="absolute lg:right-4 top-1 lg:top-auto w-2 h-2 bg-blue-400 rounded-full animate-pulse"></div>}
            </Link>
          ))}
        </nav>

        {/* Enhanced Profile - Hidden on mobile */}
        <div className="hidden lg:block relative group mt-auto">
          <div className="absolute inset-0 bg-gradient-to-r from-orange-500/20 to-red-500/20 rounded-2xl blur-xl group-hover:blur-2xl transition-all duration-300"></div>
