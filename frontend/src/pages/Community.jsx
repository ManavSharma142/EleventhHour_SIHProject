...
// function ChatSection({ messages, newMessage, setNewMessage, sendMessage, handleKeyPress, messagesEndRef, onClose }) {
//     return (
...
export default function Community() {
  ...
  useEffect(() => {
    // Only scroll when a new message is actually added, not on initial render
    if (messages.length > 7) { // 7 is the initial number of messages
      messagesEndRef.current?.scrollIntoView({ behavior: "smooth" });
    }
  }, [messages]);
